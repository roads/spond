...
class Aligner:
    ...
    def train(self, n_restart=100, max_epoch=30, hidden_size=100, gmm_scale=0.1, loss_set_scale=1.0, loss_cycle_scale=10, loss_sup_scale=10):
        ...
        best_model_f = MLP(self._n_dim, hidden_size).to(DEVICE)
        best_model_g = MLP(self._n_dim, hidden_size).to(DEVICE)
        ...
        for i_restart in range(n_restart):
            ...
            if i_restart < (n_restart - 10):  # TODO
                ...
                if rand_val[0] < thresh:
                    model_f = MLP(self._n_dim, hidden_size).to(DEVICE)
                    # print('\tNew model_f')
                else:
                    model_f = best_model_f

                if rand_val[1] < thresh:
                    model_g = MLP(self._n_dim, hidden_size).to(DEVICE)
                    # print('\tNew model_g')
                else:
            ...
            for i_epoch in range(max_epoch):
                ...
                f_x = model_f(x).to(DEVICE)
                g_y = model_g(y).to(DEVICE)
